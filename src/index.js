...
export const hex = (q, r, s = (0 - q - r)) => {
  ...
  if (Math.abs(q + r + s) > 0.0001) {
    throw new RangeError('q (' + q + '), r (' + r + ') and s (' + s + ') must sum to zero');
  }
  return { 
...
export const add = b => a => {
  return hex(a.q + b.q, a.r + b.r);
};

export const subtract = b => a => {
  return hex(a.q - b.q, a.r - b.r);
};

export const sub = subtract;

export const scale = k => a => {
...
export const direction = (direction) => {
  if (!Number.isSafeInteger(direction)) {
    throw new TypeError('direction must be an integer');
  }
  if (direction < 0 || direction >= directions.length) {
    throw new RangeError('direction must be >= 0 and < ' + directions.length);
  }
  return directions[direction];
};

export const neighbor = d => hex => {
  return hex |> add(direction(d));
};
...
export const diagonalNeighbor = direction => hex => {
  return hex |> add(diagonals[direction]);
};
...
export const distance = b => a => {
  return length(a |> subtract(b));
};

export const isAdjacent = b => a => {
...
export const lineDraw = b => a => {
  ...
  for (let i = 0; i <= n; i++) {
    results.push(round(aNudge |> lerp(bNudge, step * i)));
  }
  return results;
};
